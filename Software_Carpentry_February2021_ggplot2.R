...
data()
# Pick ONE of the following five datasets for today's exercises:
# iris, infert, msleep, midwest, diamonds
...
# if you use msleep/midwest/diamonds, I recommend running the following lines
# unless you're comfortable with tibbles (pick the line that includes your choice)
# mydata <- as.data.frame(midwest) 
# mydata <- as.data.frame(diamonds) 
# mydata <- na.omit(as.data.frame(msleep))
# then use the variable mydata for all following code
...
# Try varying these kinds of aesthetics with your own data!
# fill, group, label, color, shape, linetype
...
ggplot(mtcars, aes(x = gear, y = mpg)) + geom_bar(stat = "summary",
                                                  fun = "median")
...
# geom_bar function with stat = summary

ggplot(mtcars, aes(x = gear, y = mpg)) + 
  stat_summary(geom = "bar", fun = mean, 
               position = "dodge")
# stat_summary function with geom = bar
